...
class BIN_INFO {
    ...
    void init_bco(const int idx, const std::string bco_type) {
        switch (MBCO.at(bco_type)) {
            ...
            case NODES::NS:
                ...
                break;
            default:
                std::cerr << "Static switch not defined for " << bco_type
                          << std::endl;
    ...
    BIN_INFO(const BIN_INFO& b)
        ...
        for (int i = 0; i < 2; ++i) {
            if (b.BCOS[i]) {
                ...
                if (b_type == "bh") {
                    auto child_ptr =
                        dynamic_cast<BCO_BH_INFO*>(b.BCOS[i].get());
                    BCOS[i] = std::make_unique<BCO_BH_INFO>(*child_ptr);
                } else if (b_type == "ns") {
                    auto child_ptr =
                        dynamic_cast<BCO_NS_INFO*>(b.BCOS[i].get());
   ...
   */
    ...
    void set_defaults(config_t& bconfig) {
        ...
        bconfig.set(BIN_PARAMS::QPIG) =
            bconfig(BCO_PARAMS::BCO_QPIG, NODES::BCO1);
