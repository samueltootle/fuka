...
std::ostream& operator<<(std::ostream& out,
                         const kadath_config_boost<ParamC>& config);
...
struct kadath_config_boost : public configurator_base {
   ...
   */
    ...
    const std::string space_filename()
        const;  ///< space filename including path
   ...
   */
    void initialize_binary(std::array<std::string, 2> bco_types) {
        container.init_binary(bco_types);
    }
   ...
   */
    ...
    /**
   * kadath_config_boost::set_stage_map
   * Set stage map of base parameter container to control
   * which stages are shown in the output file
   */
    void set_stage_map(const std::map<std::string, STAGES>& map) {
        container.set_stage_map(map);
    }
   ...
   */
    ...
    /**
   * @brief The following functions are used to set and get parameters
   * from a std::tuple.  This is needed to allow for an easy interface
   * for use with parameter sequence containers
   */
    ...
    constexpr auto& operator()(std::tuple<T...> const& t,
        ...
        return this->operator()(
            std::get<I>(std::forward<std::tuple<T...>>(tt))...);
    }
   ...
   */
    ...
    friend std::ostream& operator<< <>(std::ostream&,
                                       const kadath_config_boost<ParamC>&);
};
