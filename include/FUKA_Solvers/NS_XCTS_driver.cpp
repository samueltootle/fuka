...
inline NS_XCTS_BASE::base_config_t ns_xcts_sequence_setup(NS_XCTS_BASE::base_config_t &seqconfig,
...
int ns_xcts_seq_driver(NS_XCTS_BASE::base_config_t &seqconfig, ns_sequence const &seq,
    ...
    const double final_MADM =
        (seq.mass_idx() == BCO_PARAMS::MADM) ? base_config(BCO_PARAMS::MADM) : std::nan("1");
    ...
    if (bconfig.control(CONTROLS::SEQUENCES) || bconfig.control(CONTROLS::RESOLVE)) {
        ...
        if (bconfig.control(CONTROLS::ITERATIVE_M) && last_stage_idx == STAGES::NOROT_BC) {
            std::stringstream ss;
...
inline int ns_xcts_driver(NS_XCTS_BASE::base_config_t &bconfig, ns_sequence const &seq,
    ...
    if (stage_enabled[STAGES::UNIFORM_ROT]) {
        ...
        NS_XCTS_UNIFORM_ROT<eos_t> uniformrot_solver(&bconfig, tmp_seq, resolution, outputdir,
                                                     rank);
        auto const &spinup(uniformrot_solver.get_spinup());
        bool check = (spinup && spinup->is_set() && spinup->is_varying());
        do {
