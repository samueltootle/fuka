...
NS_XCTS_DIFF_ROT<eos_t>::NS_XCTS_DIFF_ROT(NS_XCTS_BASE::base_config_t *config_,
...
std::string NS_XCTS_DIFF_ROT<eos_t>::converged_filename(const std::string stage) const {
    ...
    ss << (*bconfig)(BCO_PARAMS::NSHELLS) << "." << std::setfill('0') << std::setw(2) << res;
    return ss.str();
}
...
void NS_XCTS_DIFF_ROT<eos_t>::setup_syst() {
    ...
    for (int d = 0; d < ndom; d++) {
        ...
        case 1:
            ...
            syst->add_def(d, "eqP    = delta * D^i D_i P + A_ij * A^ij / P^7 / 8 * delta + 4piG / "
                             "2. * P^5 * Etilde");
            syst->add_def(d,
                          "eqNP   = delta * D^i D_i NP - 7. / 8. * NP / P^8 * delta * A_ij *A^ij "
...
#define FORMAT std::setw(13) << std::left << std::showpos
    ...
    std::cout << "=======================================" << std::endl
              ...
              << FORMAT << "Madm_ql: " << Madmalt << " [" << std::abs(Madm - Madmalt) / Madm << "]"
              << std::endl
              << FORMAT << "Mk: " << Mk << " [" << std::abs(Madm - Mk) / Madm << "]" << std::endl
              << FORMAT << "R: " << rs[0] << " " << rs[1] << " [" << rs[0] / rs[1] << "]"
              << std::endl;
...
void NS_XCTS_DIFF_ROT<eos_t>::initialize_diffrot_params() {
    ...
    if (law == "keh") {
        ...
        diffrot_params[DIFFROT_PARAMS::DIFF_ARATIO] =
            (*bconfig).template diffrot<double>(DIFFROT_PARAMS::DIFF_ARATIO);
        diffrot_params[DIFFROT_PARAMS::DIFF_RRATIO] =
