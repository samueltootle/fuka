...
inline NS_XCTS_BASE::NS_XCTS_BASE()
    : rank(0.), verbosity(0), ndom(-1), bconfig(nullptr), basis(nullptr), fmet(nullptr),
      cfields(nullptr), coord_vectors(nullptr), seq(nullptr), resolution(nullptr), syst(nullptr),
      conformal_factor(nullptr), lapse(nullptr), shift(nullptr), logh(nullptr), diff_omega(nullptr),
      outputdir("") {
}
...
inline NS_XCTS_BASE::NS_XCTS_BASE(NS_XCTS_BASE::base_config_t *config_, ns_sequence const &seq_,
    ...
    : rank(rank_), verbosity(0), ndom(-1), bconfig(config_), basis(nullptr), fmet(nullptr),
      ...
      lapse(nullptr), shift(nullptr), logh(nullptr), diff_omega(nullptr),
      seq(new ns_sequence(seq_)), resolution(new Parameter_sequence<BCO_PARAMS>(res_)),
...
inline void NS_XCTS_BASE::regrid() {
    ...
    if (rank == 0) {
        std::cout << "Resolution of old space: " << space->get_domain(0)->get_nbr_points()(0)
                  << " (r), " << space->get_domain(0)->get_nbr_points()(1) << " (theta), "
        ...
        std::cout << "Resolution of new space: " << res(0) << " (r), " << res(1) << " (theta), "
                  << res(2) << " (phi)" << std::endl;
        ...
        const Domain_shell_outer_adapted *new_outer_adapted =
            dynamic_cast<const Domain_shell_outer_adapted *>(new_space.get_domain(1));
        const Domain_shell_inner_adapted *new_inner_adapted =
            dynamic_cast<const Domain_shell_inner_adapted *>(new_space.get_domain(2));
        ...
        if (diff_omega) {
            ...
            bconfig->set_field(Kadath::FUKA_Config::BCO_FIELDS::DIFF_OMEGA) = true;
            Kadath::bco_utils::save_to_file(new_space, *bconfig, new_conf, new_lapse, new_shift,
                                            new_logh, new_diff_omega);
        } else {
