...
inline void NS_XCTS_BASE::save_to_file() const {
  ...
  if (diff_omega) {
    ...
    Kadath::bco_utils::save_to_file(*space, *bconfig, *conformal_factor, *lapse,
                                    *shift, *logh, *diff_omega);
  } else {
    Kadath::bco_utils::save_to_file(*space, *bconfig, *conformal_factor, *lapse,
                                    *shift, *logh);
  }
...
inline void NS_XCTS_BASE::regrid() {
  ...
  if (rank == 0) {
    std::cout << "Resolution of old space: "
              ...
              << space->get_domain(0)->get_nbr_points()(2) << " (phi)"
              << std::endl;
    ...
    if (diff_omega) {
      ...
      bconfig->set_field(Kadath::FUKA_Config::BCO_FIELDS::DIFF_OMEGA) = true;
      Kadath::bco_utils::save_to_file(new_space, *bconfig, new_conf, new_lapse,
                                      new_shift, new_logh, new_diff_omega);
    } else {
...
void NS_XCTS_BASE::load_solution_from_file() {
  ...
  if (bconfig->field(Kadath::FUKA_Config::BCO_FIELDS::DIFF_OMEGA)) {
    ...
    *diff_omega = ((*bconfig)(BCO_PARAMS::OMEGA) == 0.)
                      ? 1e-7
