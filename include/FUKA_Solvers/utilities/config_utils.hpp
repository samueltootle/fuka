...
inline void initialize_binary_config_basics(config_t& bconfig, double const M1, double const M2) {
    ...
    bconfig.set(BIN_PARAMS::COM) = com_estimate(bconfig(BIN_PARAMS::DIST), M1, M2);
}

template <class config_t>
inline void initialize_binary_inspiral_config(config_t& bconfig, double const M1, double const M2) {
    using namespace ::Kadath::bco_utils;
    using namespace ::Kadath::FUKA_Config;
