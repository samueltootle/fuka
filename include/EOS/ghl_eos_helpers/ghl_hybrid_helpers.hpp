...
inline double ghl_hybrid_rho__h_cold(
  ...
  ghl_hybrid_compute_P_cold_and_eps_cold(ghl_eos_params.get(), rhomin,
                                         &press_min, &epsmin);
  ghl_hybrid_compute_P_cold_and_eps_cold(ghl_eos_params.get(), rhomax,
                                         &press_max, &epsmax);
  ...
  const auto func = [&](const double& lrho) {
    ...
    ghl_hybrid_compute_P_cold_and_eps_cold(ghl_eos_params.get(), rho,
                                           &press_cold, &eps_cold);
