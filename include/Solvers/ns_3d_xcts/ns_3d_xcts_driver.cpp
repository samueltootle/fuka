    ...
    auto single_seq = [&](auto val) {
        ...
        if (std::get<0>(sequence_var_indices) == BCO_PARAMS::CHI) {
            ...
            if (std::fabs(old_val) < 0.1 && val > 0.2) {
                ...
                exit_status =
                    ns_3d_xcts_base_solution_driver(bconfig, outputdir);
            }
        }
...
int ns_3d_xcts_stationary_driver(config_t& bconfig,
    ...
    exit_status = EOS_Function_Dispatcher::dispatch<launch_ns_solver>(bconfig,
                                                                      eos_type,
                                                                      rank,
                                                                      bconfig,
                                                                      outputdir,
...
inline int ns_3d_xcts_driver(config_t& bconfig,
    ...
    while (res_inc) {
        ...
        if (rank == 0)
            std::cout << "Increasing resolution to: "
                      << bconfig(BCO_PARAMS::BCO_RES) << "\n";
        regrid();
...
inline int ns_3d_xcts_binary_boost_driver(
    ...
    while (exit_status == RUN_BOOST) {
        exit_status =
            EOS_Function_Dispatcher::dispatch<launch_ns_boost_solver>(bconfig,
                                                                      eos_type,
                                                                      rank,
                                                                      bconfig,
                                                                      outputdir,
