...
std::string bh_3d_xcts_solver<config_t, space_t>::converged_filename(
  ...
  ss << bconfig(MCH) << "." << bconfig(CHI) << "." << std::setfill('0')
     << std::setw(1) << bconfig(NSHELLS) << "." << std::setfill('0')
     << std::setw(2) << res;
  return ss.str();
}
...
template <typename config_t, typename space_t>
void bh_3d_xcts_solver<config_t, space_t>::print_diagnostics_norot(
...
template <typename config_t, typename space_t>
void bh_3d_xcts_solver<config_t, space_t>::print_diagnostics(
