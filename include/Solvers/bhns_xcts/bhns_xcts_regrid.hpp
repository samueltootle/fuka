...
inline int bhns_xcts_regrid(config_t& bconfig, std::string output_fname) {
  ...
  Space_bhns space(type_coloc, bconfig(DIST), NS_bounds, BH_bounds, out_bounds,
                   bconfig(BIN_RES), bconfig(NINSHELLS, BCO1));
  ...
  update_adapted_field(old_conf, old_space.ADAPTEDBH + 1, old_space.ADAPTEDBH,
                       old_bh_outer, OUTER_BC);
  update_adapted_field(old_lapse, old_space.ADAPTEDBH + 1, old_space.ADAPTEDBH,
                       old_bh_outer, OUTER_BC);
  for (int i = 1; i < 4; ++i)
    update_adapted_field(old_shift.set(i), old_space.ADAPTEDBH + 1,
