...
std::string bns_xcts_solver<eos_t, config_t, space_t>::converged_filename(
  ...
  ss << eosname << "." << bconfig(DIST) << "." << bconfig(CHI, BCO1) << "."
     << bconfig(CHI, BCO2) << "." << bconfig(MADM, BCO1) + bconfig(MADM, BCO2)
     << ".q" << bconfig(Q) << "." << std::setfill('0') << std::setw(1)
     << bconfig(NSHELLS, BCO1) << "." << std::setfill('0') << std::setw(1)
     << bconfig(NSHELLS, BCO2) << "." << std::setfill('0') << std::setw(2)
     << res;
  return ss.str();
}
...
template <class eos_t, typename config_t, typename space_t>
void bns_xcts_solver<eos_t, config_t, space_t>::print_diagnostics(
