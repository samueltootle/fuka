...
struct bns_setup_boosted_3d {
    inline void operator()(kadath_config_boost<BCO_NS_INFO>& NS1config,
            ...
            [&](auto& spacein, auto& old_space_radius, const int ndomin) {
                ...
                const Domain_shell_outer_adapted* old_outer_adapted =
                    dynamic_cast<const Domain_shell_outer_adapted*>(
                        spacein.get_domain(1));
                for (int i = 0; i < ndomin; ++i)
                    if (i != 1)
                        old_space_radius.set_domain(i) =
                            spacein.get_domain(i)->get_radius();
                old_space_radius.set_domain(1) =
        ...
        {
            auto drPsi(
                compute_drPsi(spacein1,
                              confin1,
                              Metric_flat(spacein1, shiftin1.get_basis()),
                              {0, 1}));
            NS1_bounds = set_arb_boundsv3(bconfig, drPsi, 2, NODES::BCO1);
        }
        std::vector<double> NS2_bounds;
        {
            auto drPsi(
                compute_drPsi(spacein2,
                              confin2,
                              Metric_flat(spacein2, shiftin2.get_basis()),
                              {0, 1}));
            NS2_bounds = set_arb_boundsv3(bconfig, drPsi, 2, NODES::BCO2);
        }
        ...
        if (rank == 0)
            std::cout << "WeightNS1: "
                      << bconfig(BCO_PARAMS::DECAY, NODES::BCO1) << ", "
                      << "WeightNS2: "
                      << bconfig(BCO_PARAMS::DECAY, NODES::BCO2) << std::endl;
...
void bns_xcts_superimposed_import(config_t& bconfig,
    ...
    const std::string eos_type =
        NS1config.eos<std::string>(EOS_PARAMS::EOSTYPE);
    EOS_Function_Dispatcher::dispatch<bns_setup_boosted_3d>(bconfig,
                                                            eos_type,
                                                            NS1config,
                                                            NS2config,
                                                            bconfig);
