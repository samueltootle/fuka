...
int bns_xcts_regrid(config_t& bconfig, std::string output_fname) {
    ...
    Scalar drPsi(compute_drPsi(old_space,
                               old_conf,
    ...
    const std::array<double, 2> xc{space.get_domain(space.NS1)->get_center()(1),
                                   space.get_domain(space.NS2)->get_center()(
                                       1)};
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        new_outer_adapted[i] = dynamic_cast<const Domain_shell_outer_adapted*>(
            space.get_domain(d));
