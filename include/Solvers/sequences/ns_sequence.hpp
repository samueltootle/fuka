...
class ns_sequence : public seq_t {
    ...
    */
    ns_sequence (std::string _str, std::tuple<BCO_PARAMS> _t) : seq_t(_str, _t) {}
    ...
    std::string mass_str() const {
        auto [ mass_key, mass_idx ] = get_key_val_pair_from_val(MBCO_PARAMS, mass_fixing_idx);
        return mass_key;
    }
...
void parse_tree_for_fixed(Tree const & tree, std::string const branch_name, 
    ...
    if(!std::isnan(_val)) {
        if(ns_idx_is_mass_fixing(idx) && seq.is_mass_set()) {
            cout << std::to_string(idx) << ", " << std::to_string(seq.mass_val()) << '\n';
            std::string msg{"Mass can only be fixed by one value. Check your config!\n"};
            throw std::runtime_error(msg.c_str());
        } else if(ns_idx_is_spin_fixing(idx) && seq.is_spin_set()) {
            std::string msg{"Spin can only be fixed by one value. Check your config!\n"};
            throw std::runtime_error(msg.c_str());
...
std::ostream &operator<<(std::ostream & out, const ns_sequence & Seq) {
  ...
  auto [ mass_key, mass_idx ] = get_key_val_pair_from_val(MBCO_PARAMS, Seq.mass_idx());
  auto [ spin_key, spin_idx ] = get_key_val_pair_from_val(MBCO_PARAMS, Seq.spin_idx());
  out << std::setw(20) << mass_key << "(" << mass_idx<<"): " << Seq.mass_val() << '\n'
      << std::setw(20) << spin_key << "(" << spin_idx<<"): " << Seq.spin_val() << '\n'
