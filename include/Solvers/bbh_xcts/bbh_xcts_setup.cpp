...
inline void bbh_xcts_setup_boosted_3d(
    ...
    {
        auto drPsi(compute_drPsi(spacein1,
                                 confin1,
    ...
    {
        auto drPsi(compute_drPsi(spacein2,
                                 confin2,
    ...
    auto interp_BH_fields =
        [&](auto& bhconf, auto& bhlapse, auto& bhshift, auto& bhspacein) {
            ...
            for (int i = 1; i <= 3; ++i)
                bco_u::update_adapted_field(bhshift.set(i),
                                            2,
                                            1,
                                            old_bh_outer,
                                            OUTER_BC);
    ...
    for (int dom = 0; dom < ndom; dom++) {
        ...
        do {
            if (dom <= space.BH1 + 1 || dom == space.BH2 ||
                dom == space.BH2 + 1)
                continue;
            ...
            if (dom < ndom - 1) {
                ...
                for (int i = 1; i <= 3; i++)
                    shift.set(i).set_domain(dom).set(new_pos) =
                        decay_1 * shiftin1(i).val_point(absol1) +
                        decay_2 * shiftin2(i).val_point(absol2);

            } else {
                // We have to set the compactified domain manually since the outer
                // collocation point is always at inf which is undefined numerically
