...
inline void update_bin_config(config_t& bconfig,
    ...
    if (!bconfig.control(USE_CONFIG_VARS)) {
        ...
        const double r_max_tot = (bconfig(BCO_PARAMS::RMID, NODES::BCO1) >
...
inline int bbh_xcts_regrid(config_t& bconfig, std::string outputfile) {
    ...
    if (!bconfig.control(USE_CONFIG_VARS)) {
        bco_u::update_config_BH_radii(old_space,
                                      bconfig,
                                      old_space.BH1 + 1,
                                      old_conf,
                                      NODES::BCO1);
    ...
    std::vector<int> exclusion_doms{old_space.BH1,
                                    old_space.BH1 + 1,
                                    old_space.BH2,
                                    old_space.BH2 + 1};
    ...
    for (auto& i : {0, 1}) {
        ...
        bco_u::update_adapted_field(old_conf,
                                    old_nuc_doms[i] + 2,
                                    old_nuc_doms[i] + 1,
                                    old_outer_homothetic[i],
                                    OUTER_BC);
        bco_u::update_adapted_field(old_lapse,
                                    old_nuc_doms[i] + 2,
                                    old_nuc_doms[i] + 1,
                                    old_outer_homothetic[i],
                                    OUTER_BC);
        for (int j = 1; j < 4; ++j)
            bco_u::update_adapted_field(old_shift.set(j),
                                        old_nuc_doms[i] + 2,
                                        old_nuc_doms[i] + 1,
                                        old_outer_homothetic[i],
                                        OUTER_BC);
