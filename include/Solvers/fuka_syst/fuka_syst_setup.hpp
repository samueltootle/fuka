 ...
 * @param syst System of equations to modify
 */
inline void syst_init_defs(System_of_eqs& syst) {
 ...
 * @param syst System of equations to modify
 */
inline void syst_init_contraction_defs_vac(System_of_eqs& syst) {
#ifdef DEBUG
 ...
 * @param CART coordinate field relative to the coordinate distance to the
 * center of mass
 */
...
void syst_init_binary(System_of_eqs& syst,
                      cfields_t& coord_vectors,
                      config_t& bconfig) {
#ifdef DEBUG
    std::cout
        << "Initializing standard binary fields, constants, and definitions.\n";
#endif
    int const ndom = syst.get_space().get_nbr_domains();
 ...
 */
inline void syst_init_quasi_local_defs(System_of_eqs& syst,
                                       std::vector<int> doms,
                                       std::string rotdef,
                                       std::string surfdef) {
#ifdef DEBUG
...
#endif
    std::string qlspin_def = "intS = A_ij * " + rotdef + "^i * " + surfdef +
                             "^j";
    qlspin_def += " / 2 / 4piG";
    for (auto& d : doms) {
...
#ifdef DEBUG
    std::cout
        << "Initializing standard CO fields, constants, and definitions.\n";
#endif
    int const ndom = syst.get_space().get_nbr_domains();
 ...
 * @param doms vector of domains to populate these definitions
 */
inline void syst_init_eqdefs_vac(System_of_eqs& syst, std::vector<int> doms) {
#ifdef DEBUG
    ...
    for (auto& d : doms) {
        ...
        syst.add_def(d,
                     "eqbet^i = D_j D^j bet^i \
                          + D^i D_j bet^j / 3. - 2. * A^ij * D_j Ntilde");
    }
}

template <class config_t>
inline std::string set_ns_mass_fixing(
    System_of_eqs& syst,
    config_t& bconfig,
    std::unique_ptr<Kadath::FUKA_Solvers::ns_sequence const>& seq) {
...
template <class config_t>
inline std::string get_ns_mass_fixing_output(
    config_t& bconfig,
    std::unique_ptr<Kadath::FUKA_Solvers::ns_sequence const>& seq) {
...
template <class config_t>
inline std::string set_ns_spin_fixing(
    System_of_eqs& syst,
    config_t& bconfig,
    std::unique_ptr<Kadath::FUKA_Solvers::ns_sequence const>& seq) {
    ...
    switch (idx) {
        ...
        case BCO_PARAMS::JADM:
            ...
            bconfig.set(BCO_PARAMS::CHI) = bconfig(BCO_PARAMS::JADM) /
        ...
        default:
            std::string msg{
                "Sequence initialized, but not implemented for Spin index = " +
                std::to_string(int(idx))};
            throw std::runtime_error(msg.c_str());
            break;
    }
...
template <class config_t>
inline std::string get_ns_spin_fixing_output(
    config_t& bconfig,
    std::unique_ptr<Kadath::FUKA_Solvers::ns_sequence const>& seq) {
    std::stringstream output;
    ...
    switch (idx) {
        case BCO_PARAMS::OMEGA:
            output << "rotation velocity (omega) = "
                   << bconfig(BCO_PARAMS::OMEGA);
            break;
        case BCO_PARAMS::JADM:
            output << "ADM Angular Momentum (jadm) = "
                   << bconfig(BCO_PARAMS::JADM);
            break;
        case BCO_PARAMS::CHI:
            output << "dimensionless spin parameter (chi) = "
