...
class Solver {
 ...
 protected:
  virtual void print_diagnostics(const System_of_eqs& syst,
                                 const int ite,
   ...
   */
  void check_max_iter_exceeded(const int& rank,
                               const int& ite,
  ...
  bool solution_exists(std::string last_stage = "") {
    ...
    else if (bconfig.control(SAVE_COS) && fs::exists(central_abs + ".info") &&
