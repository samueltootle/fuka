...
config_t ns_isotropic_sequence(config_t& seqconfig,
    ...
    if (bconfig.control(CONTROLS::SEQUENCES) ||
        ...
        int n_shells = (std::isnan(bconfig.set(BCO_PARAMS::NSHELLS)))
                           ? 0
                           : bconfig.set(BCO_PARAMS::NSHELLS);
        // the 2D code is very sensitive to the initial domain decomposition.
        // Although polytropic laws are very stable, tabulated solution are more
        // sensitive.
        ...
        if (bconfig.control(CONTROLS::ITERATIVE_M) &&
            std::fabs(bconfig(spin_fixing)) < 1e-3) {
            if (rank == 0)
    ...
    auto single_seq = [&](auto val) {
        ...
        if (std::get<0>(sequence_var_indices) == BCO_PARAMS::CHI) {
            ...
            if (std::fabs(old_val) < 0.1 && val > 0.2) {
                bconfig.control(CONTROLS::SEQUENCES) = true;
                exit_status =
                    ns_isotropic_driver(bconfig, resolution, outputdir, &seq);
            }
        }
        exit_status = ns_isotropic_driver(bconfig, resolution, outputdir, &seq);
        resolution.set(resolution.final(),
                       resolution.final(),
                       resolution.final());
        return exit_status;
    };
...
int ns_isotropic_base_solution_driver(config_t& bconfig,
                                      std::string outputdir,
    ...
    while (exit_status == RELOAD_FILE) {
        ...
        if (stage_enabled[STAGES::NOROT_BC]) {
            ...
            exit_status =
                ns_isotropic_norot_stationary_driver(bconfig, outputdir, seq);
            ...
            exit_status = ns_isotropic_uniform_rot_stationary_driver(bconfig,
                                                                     outputdir,
                                                                     seq);
            ...
            exit_status = ns_isotropic_diff_rot_stationary_driver(bconfig,
                                                                  outputdir,
                                                                  seq);
...
inline int ns_isotropic_driver(config_t& bconfig,
    ...
    std::function<int(config_t&, Res_t&, std::string, ns_sequence const*)>
        final_stage_driver;
    if (rank == 0)
    ...
    switch (last_stage_idx) {
        ...
        case STAGES::UNIFORM_ROT:
            final_stage_driver =
                &ns_isotropic_uniform_rot_driver<config_t, Res_t>;
            break;
