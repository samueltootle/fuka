...
ns_isotropic_uniform_rot_solver<eos_t, config_t, space_t>::
...
template <class eos_t, typename config_t, typename space_t>
std::string
ns_isotropic_uniform_rot_solver<eos_t, config_t, space_t>::converged_filename(
...
int ns_isotropic_uniform_rot_solver<eos_t, config_t, space_t>::solve() {
  ...
  double const initial_chi = (!bconfig.control(CONTROLS::ITERATIVE_CHI))
                                 ? bconfig(BCO_PARAMS::CHI)
                                 : std::copysign(0.1, final_chi);
...
template <class eos_t, typename config_t, typename space_t>
void ns_isotropic_uniform_rot_solver<eos_t, config_t, space_t>::syst_init(
    System_of_eqs& syst) {
  using namespace ::Kadath::Margherita;
  ...
  // This expression does not give accurate results when compared to M_komar and
  // the computed ADM Mass from the 3D code.  The deviation from the correct
  // answer gets large with increasing differential rotation profiles, but the
  // source of the error is unknown eq. 4.21 arxiv.org/abs/1003.5015v2
  // FIXME - there must be a reason
  // syst.add_def(ndom - 1, "intMadm = - (dr(A^2 + B^2) + divr(B^2 - A^2))  / 4
  // / 4piG "); Instead, the following definition gives very accurate
  // comparisons with M_komar
...
template <class eos_t, typename config_t, typename space_t>
void ns_isotropic_uniform_rot_solver<eos_t, config_t, space_t>::
    print_diagnostics(const System_of_eqs& syst,
                      const int ite,
