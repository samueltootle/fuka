   ...
   */
    ...
    struct export_pointwise_imp {
        ...
        output_ary_t operator()(CFMS_NS_ISO_Exporter& base,
            ...
            double const domega_dt =
                base.quant_vals[ISO_VARS::ISO_DOMEGA_DTHETA];
      ...
      export_utils::basis_transform_spherical_tofrom_cart::matrix_t KDD_Sph = {{
        ...
        {{ KDD_rphi, KDD_thphi, 0.0      }}
            ...
            auto const KDD_Cart =
                basis_transform.matrixDD__sph_to_cart(KDD_Sph);
            auto const betaU_Cart =
                basis_transform.vectorU__sph_to_cart(betaU_Sph);
            ...
            base.out_pw[OUTPUT_VARS::ALPHA] =
                base.quant_vals[ISO_VARS::ISO_ALPHA];
    ...
    struct export_pointwise__spherical_imp {
        ...
        output_ary_t operator()(CFMS_NS_ISO_Exporter& base,
            ...
            auto get_theta = [&]() {
                ...
                if (r2 < 1e-12) {
                    theta =
                        acos(z / 1e-10);  // theta (angle from Z to xy plane)
                } else {
            ...
            base.out_pw[OUTPUT_VARS::K13] = -base.out_pw[OUTPUT_VARS::G33] /
                                            2.0 / N * domega_dr;
            base.out_pw[OUTPUT_VARS::K22] = 0.0;
            base.out_pw[OUTPUT_VARS::K23] = -base.out_pw[OUTPUT_VARS::G33] /
                                            2.0 / N * domega_dt;
