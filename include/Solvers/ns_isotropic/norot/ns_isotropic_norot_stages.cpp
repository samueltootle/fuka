...
int ns_isotropic_norot_solver<eos_t, config_t, space_t>::norot_stage(
    bool fixed) {
  ...
  if (seq && !fixed) {
    ...
    output << "Mass fixed using central enthalpy (hc) = "
  ...
  // use the helper construction, i.e. a level function with a root defining the
  // radius
  if (fixed && bconfig.control(CONTROLS::USE_FIXED_R)) {
