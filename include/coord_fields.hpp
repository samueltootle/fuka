...
class CoordFields {
   ...
   public:
    CoordFields(space_t const& space)
        : space(space), basis(space, CARTESIAN_BASIS) {}
    ...
    Kadath::Vector cart(double shift_x = 0.,
                        double shift_y = 0.,
                        double shift_z = 0.) const;
     ...
     */
    Kadath::Scalar radius(double shift_x = 0.,
                          double shift_y = 0.,
                          double shift_z = 0.) const;
     ...
     */
    template <int ind_t = CON>
    Kadath::Vector rot_z(double shift_x = 0.,
                         double shift_y = 0.,
                         double shift_z = 0.) const;
     ...
     */
    template <int ind_t = CON>
    Kadath::Vector e_rad(double shift_x = 0.,
                         double shift_y = 0.,
                         double shift_z = 0.) const;
...
template <typename space_t>
template <int ind_t>
Kadath::Vector CoordFields<space_t>::cart(double shift_x,
                                          double shift_y,
                                          double shift_z) const {
    ...
    cart.set(1).set_domain(ndom - 1) =
        space.get_domain(ndom - 1)->get_cart_surr(1);
    cart.set(2).set_domain(ndom - 1) =
        space.get_domain(ndom - 1)->get_cart_surr(2);
    cart.set(3).set_domain(ndom - 1) =
        space.get_domain(ndom - 1)->get_cart_surr(3);
...
template <typename space_t>
Kadath::Scalar CoordFields<space_t>::radius(double shift_x,
                                            double shift_y,
                                            double shift_z) const {
...
template <typename space_t>
template <int ind_t>
Kadath::Vector CoordFields<space_t>::rot_z(double shift_x,
                                           double shift_y,
                                           double shift_z) const {
    auto coords = this->cart(shift_x, shift_y, shift_z);
...
template <typename space_t>
template <int ind_t>
Kadath::Vector CoordFields<space_t>::e_rad(double shift_x,
                                           double shift_y,
                                           double shift_z) const {
    auto coords = this->cart(shift_x, shift_y, shift_z);
    ...
    for (int i : {1, 2, 3}) {
        ...
        for (int dom = 0; dom < ndom - 1; ++dom) {
            ...
            do {
                if (!std::isfinite(e_rad(i)(dom)(pos)) && pos(0) == 0) {
                    ...
                    e_rad.set(i).set_domain(dom).set(pos) =
                        e_rad(i)(dom)(tempos);
                }
            } while (pos.inc());
        }
        ...
        e_rad.set(i).set_domain(ndom - 1) =
            space.get_domain(ndom - 1)->get_cart_surr(i);
    }
...
void update_fields(CoordFields<space_t> const& cf_generator,
    ...
    auto update = [&](auto& name, auto& field) {
        for (int dom = 0; dom < ndom; ++dom) {
            __attribute__((unused)) bool succ =
                update_field(*syst, dom, name.c_str(), field);
#ifdef DEBUG
            if (!succ)
...
inline bool update_field(System_of_eqs& syst,
                         int dom,
                         const char* n,
                         Tensor& so) {
    char* name;
