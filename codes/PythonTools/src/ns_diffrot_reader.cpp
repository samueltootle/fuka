...
class ns_reader_t : public Kadath::python_reader_t<space_t, ns_vars_t> {
  ...
  struct compute_defs {
    ...
    void operator()(kadath_config_boost<BCO_NS_INFO> bconfig,
      ...
      std::vector<int> vac_Domains{FUKA_Syst_tools::vector_of_domains(2, ndom)};
      ...
      FUKA_Syst_tools::syst_init_quasi_local_defs(syst, vac_Domains, "mg",
                                                  "sm");
      ...
      FUKA_Syst_tools::export_radii(space, this_reader->vars, 0, ndom - 1,
                                    "NS_R");
...
BOOST_PYTHON_MODULE(_ns_diffrot_reader) {
  // initialize python types
  Kadath::initPythonBinding<space_t>();
  constructPythonReader_here<ns_reader_t>("ns_diffrot_reader");
}