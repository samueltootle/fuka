...
// specialized quantities for a uniform rotating isotropic NS
...
Kadath::var_vector Kadath::vars_base_t<ns_isotropic_vars_t>::vars = {
    ...
    {"lap_Bterm", SCALAR}, {"lap_wterm", SCALAR},
};
...
class ns_isotropic_reader_t
  ...
  struct compute_defs {
    ...
    void operator()(kadath_config_boost<BCO_ISO_NS_INFO> bconfig,
      ...
      this_reader->vars["domega_dr"] = domega_dr;
      this_reader->vars["domega_dt"] = domega_dt;
      ...
      // Brsint.coef();
      this_reader->vars["Brsint"] = Brsint;
      this_reader->vars["Brsintsq"] = Brsint2;

      // Scalar one(space);
      // Scalar rsint(one.mult_r().mult_sin_theta());
      // rsint.coef();
      // this_reader->vars["rsint"] = rsint;
