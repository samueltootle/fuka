...
if __name__ == "__main__":
  ...
  for d in sorted(glob.glob("res_seq*")):
    fabs=d+"/NS_ISO_NOROT_BC.togashi.hc.1.32632.0.0.*.info"
    print(fabs)
...
#    print(files)
    ...
    for f in sorted(files):
      ...
      ns = get_reader(fID, ns_iso_norot=True)
      if not ns.config['ns']['res'] in [9, 11, 13, 17, 21, 25, 33]:
        continue
