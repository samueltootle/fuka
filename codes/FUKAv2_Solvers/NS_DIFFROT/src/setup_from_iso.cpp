...
void ISO_XCTS_convert(config_t& in_bconfig) {
    ...
    auto convert_fields= [&](const size_t dom) {
        ...
        do {
            ...
            lapse.set_domain(dom).set(pos) = all_data[input_reader_t::OUTPUT_VARS::ALPHA];
            
            // approximation
            auto const psi4 = all_data[input_reader_t::OUTPUT_VARS::G11];
...
int main(int argc, char **argv) {
  ...
  if(eos_type == "Cold_PWPoly") {
    ...
    ISO_XCTS_convert<eos_t>(bconfig);
