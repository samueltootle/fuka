...
int main(int argc, char** argv) {
  ...
  if (!bconfig.control(USE_CONFIG_VARS)) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      bco_utils::set_radius(old_adapted_doms[i], old_space, bconfig, FIXED_R,
                            i);
    }

    // update NS radii
    auto [rmin, rmax] =
        bco_utils::get_rmin_rmax(old_space, old_space.ADAPTEDNS);
    ...
    auto [cmin, cmax] =
        bco_utils::get_field_min_max(old_conf, old_space.ADAPTEDBH + 1,
                                     INNER_BC);
  ...
  BH_bounds[1] =
      bco_utils::get_radius(old_space.get_domain(old_space.BH + 1), OUTER_BC);
  // end setup bounds
  ...
  Space_bhns space(type_coloc, bconfig(DIST), NS_bounds, BH_bounds, out_bounds,
                   bconfig(BIN_RES), bconfig(NINSHELLS));
  ...
  bco_utils::update_adapted_field(old_conf, old_space.ADAPTEDBH + 1,
                                  old_space.ADAPTEDBH, old_bh_outer, OUTER_BC);
  bco_utils::update_adapted_field(old_lapse, old_space.ADAPTEDBH + 1,
                                  old_space.ADAPTEDBH, old_bh_outer, OUTER_BC);
  for (int i = 1; i < 4; ++i)
    bco_utils::update_adapted_field(old_shift.set(i), old_space.ADAPTEDBH + 1,
                                    old_space.ADAPTEDBH, old_bh_outer,
                                    OUTER_BC);

  bco_utils::update_adapted_field(old_phi, old_space.ADAPTEDNS,
                                  old_space.ADAPTEDNS + 1, old_inner_adaptedNS,
                                  INNER_BC);

  // Updated mapping for NS adapted fields
  bco_utils::interp_adapted_mapping(new_ns_inner, old_space.ADAPTEDNS,
                                    old_space_radius);
  bco_utils::interp_adapted_mapping(new_ns_outer, old_space.ADAPTEDNS,
                                    old_space_radius);
