...
void print_diagnostics(Space_bin_ns const& space,
...
template <typename config_t>
std::stringstream converged_filename(const std::string& stage,
                                     config_t bconfig);
...
int BNS_solver(config_t& bconfig, std::string outputdir) {
  ...
  if (stage_enabled[TOTAL]) {
    if (rank == 0)
      std::cout << "############################" << std::endl
                ...
                << bconfig(CHI, BCO2) << " and q = " << bconfig(Q) << std::endl
                << "############################" << std::endl;
    ...
    if (!bconfig.control(COROT_BIN)) {
      ...
      if (std::isnan(bconfig.set(FIXED_BCOMEGA, BCO1)))
        space.add_eq_int_outer_sphere_one(
            syst, "integ(intS1) / Madm1 / Madm1 = chi1");

      if (std::isnan(bconfig.set(FIXED_BCOMEGA, BCO2)))
        space.add_eq_int_outer_sphere_two(
            syst, "integ(intS2) / Madm2 / Madm2 = chi2");
    }
    ...
    syst.add_eq_first_integral(space.NS1, space.ADAPTED1, "firstint",
                               "H - Hc1");
    syst.add_eq_first_integral(space.NS2, space.ADAPTED2, "firstint",
                               "H - Hc2");
  ...
  if (stage_enabled[TOTAL_BC]) {
    ...
    while (!endloop) {
      ...
      if (rank == 0) {
        print_diagnostics(space, syst, ite, conv, bconfig);
        std::cout << "Rescale Factors: " << H_scale1 << " " << H_scale2
                  << std::endl;
        if (bconfig.control(CHECKPOINT))
          bco_utils::save_to_file(space, bconfig, conf, lapse, shift, logh,
                                  phi);
...
#define FORMAT std::setw(13) << std::left << std::showpos
  ...
  std::cout
      ...
      << FORMAT << "Omega: " << bconfig(GOMEGA)
      << std::endl
      // "center of mass" shift
      << FORMAT << "Axis: " << bconfig(COM)
      << std::endl
...
std::stringstream converged_filename(const std::string& stage,
  ...
  ss << bconfig(DIST) << "." << bconfig(CHI, BCO1) << "." << bconfig(CHI, BCO2)
     << "." << bconfig(MADM, BCO1) + bconfig(MADM, BCO2) << ".q" << bconfig(Q)
