    ...
    for (int d = 0; d < ndom; d++) {
        switch (d) {
            ...
            case 1:
                ...
                syst.add_def(
                    d,
                    "E = Wsq * (press * h - Wsq * press * delta / Wsq)");
                ...
                syst.add_def(d,
                             "intDDA = - lap(A) * multrsint(A^2) * multr(B)");
    ...
    for (int d = 0; d < ndom; d++) {
        switch (d) {
            ...
            case 1:
                ...
                syst.add_def(d,
                             "intDDA = - lap2(A) * multrsint(A^2) * multr(B)");
            ...
            default:
                ...
                syst.add_def(d, "intDDA = - lap2(A) * 2 / 4piG");
                syst.add_def(d,
                             "GRV2 = divrsint(scal(grad(nu), grad(nu))) / 2");

                syst.add_def(d,
                             "eqnu = lap(nu) + scal(grad(nu), grad(lapAterm))");
                syst.add_def(
                    d,
...
int main(int argc, char** argv) {
    ...
    EOS_Function_Dispatcher::dispatch<reader_select>(bconfig,
                                                     eos_type,
                                                     bconfig);
