...
struct bns_reader {
    ...
    void operator()(config_t& bconfig) {
        ...
        const std::array<double, 2> x_nuc{bco_utils::get_center(space,
                                                                space.NS1),
                                          bco_utils::get_center(space,
                                                                space.NS2)};
        ...
        syst.add_def(ndom - 1,
                     "intCOMx = 3. / 2. / 4piG * P^4 * ex_i * einf^i");
        syst.add_def(ndom - 1,
                     "intCOMy = 3. / 2. / 4piG * P^4 * ey_i * einf^i");
        syst.add_def(ndom - 1,
                     "intCOMz = 3. / 2. / 4piG * P^4 * ez_i * einf^i");

        // quasi-local spin angular momentum surface integral integrants for both stars
        syst.add_def(space.ADAPTED1 + 1,
                     "intS = A_ij * mm^i * sm^j / 2. / 4piG");
        syst.add_def(space.ADAPTED2 + 1,
                     "intS = A_ij * mp^i * sp^j / 2. / 4piG");
        ...
        for (int d = 0; d < ndom; d++) {
            if ((d >= space.ADAPTED2 + 1) ||
                ...
                if (bconfig.control(COROT_BIN)) {
                    ...
                    syst.add_def(d,
                                 "firstint = log(h * N / W + D_i phi * V^i)");
                }
        ...
        syst.add_def(ndom - 1,
                     "intJ = multr(A_ij * Morb^j * einf^i) / 2. / 4piG");
        ...
        for (int i : {0, 1}) {
            ...
            double A =
                space.get_domain(dom + 1)->integ(syst.give_val_def("intMsq")()(
                                                     dom + 1),
                                                 INNER_BC);
            ...
            central_rho[i] =
                EOS<eos_t, DENSITY>::get(std::exp(central_logh[i]));
            ...
            for (int d = nuc_doms[i]; d <= adapted_doms[i]; ++d) {
                ...
                baryonic_mass.push_back(
                    syst.give_val_def("intMb")()(d).integ_volume());
            }
            ...
            ql_spin[i] =
                space.get_domain(dom + 1)->integ(syst.give_val_def("intS")()(
                                                     dom + 1),
                                                 OUTER_BC);
        ...
        // binary quantities
        double adm_inf =
            space.get_domain(ndom - 1)->integ(syst.give_val_def("Madm")()(ndom -
                                                                          1),
                                              OUTER_BC);
        double komar = space.get_domain(ndom - 1)->integ(syst.give_val_def(
                                                             "Mk")()(ndom - 1),
                                                         OUTER_BC);
        double Jinf = space.get_domain(ndom - 1)->integ(syst.give_val_def(
                                                            "intJ")()(ndom - 1),
                                                        OUTER_BC);
        double Px = space.get_domain(ndom - 1)->integ(syst.give_val_def(
                                                          "intPx")()(ndom - 1),
                                                      OUTER_BC);
        double Py = space.get_domain(ndom - 1)->integ(syst.give_val_def(
                                                          "intPy")()(ndom - 1),
                                                      OUTER_BC);
        double Pz = space.get_domain(ndom - 1)->integ(syst.give_val_def(
                                                          "intPz")()(ndom - 1),
                                                      OUTER_BC);

        // center of mass defined like in https://arxiv.org/abs/1506.01689
        double COMx =
            space.get_domain(ndom - 1)->integ(syst.give_val_def("intCOMx")()(
                                                  ndom - 1),
                                              OUTER_BC) /
            adm_inf;
        double COMy =
            space.get_domain(ndom - 1)->integ(syst.give_val_def("intCOMy")()(
                                                  ndom - 1),
                                              OUTER_BC) /
            adm_inf;
        double COMz =
            space.get_domain(ndom - 1)->integ(syst.give_val_def("intCOMz")()(
                                                  ndom - 1),
                                              OUTER_BC) /
            adm_inf;
    ...
    std::setw(25) << std::right << std::setprecision(5) << std::scientific \
        ...
        auto print_shells = [&](int dom_min, int dom_max) {
            ...
            for (int i = dom_min; i < dom_max; ++i) {
                ...
                std::cout << FORMAT1 << shell << "["
                          << bco_utils::get_radius(space.get_domain(i),
                                                   INNER_BC)
                          << ", "
        ...
        for (int i = 0; i <= 1; ++i) {
            ...
            std::cout << FORMAT1 << "Center_COM = " << "(" << xcom[i]
                      << ", 0, 0)\n"
                      << FORMAT1 << "Coord R_IN = "
                      << bco_utils::get_radius(space.get_domain(nuc_doms[i]),
                                               EQUI)
                      << '\n'
                      << FORMAT1 << "Coord R = " << "[" << r_extrema[i][0]
                      << "," << r_extrema[i][1] << "] (" << "["
                      << r_extrema[i][0] * M2km << "," << r_extrema[i][1] * M2km
                      << "] km)\n";
            print_shells(adapted_doms[i] + 1, bounds[i]);
            std::cout
                << FORMAT1 << "Coord R_OUT = "
                << bco_utils::get_radius(space.get_domain(adapted_doms[i] + 1),
                                         EQUI)
                << "\n"
                << FORMAT1 << "Areal R = " << areal_r[i] << " ["
                << areal_r[i] * M2km
                << "km]\n"
        ...
        std::cout
            ...
            << "Separation = " << bconfig(DIST) << " [" << bconfig(DIST) / Minf
            << "] (" << bconfig(DIST) * M2km << "km)"
            << std::endl
            ...
            << ", Diff: " << fabs(adm_inf - komar) / (adm_inf + komar)
            << std::endl
