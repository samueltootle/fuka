...
struct reader_3d {
    ...
    void operator()(config_t& bconfig) {
        ...
        // baryonic mass, dimensionless spin, central log enthalpy, angular
        // frequency parameter
        ...
        syst.add_def(
            ndom - 1,
            "intMk = (einf^i * D_i N - A_ij * einf^i * bet^j) / 4 / PI");
        ...
        double Madmalt =
            space.get_domain(ndom - 1)->integ(integMadmalt, OUTER_BC);
        ...
        double A = space.get_domain(2)->integ(syst.give_val_def("intMsq")()(2),
                                              INNER_BC);
        ...
        double Circumferential_R = conf_eq * conf_eq *
                                   space.get_domain(1)->get_radius()(pos_eq);
        double& CR = Circumferential_R;

        Scalar logh_dr(logh.der_r());
        double mass_shedding_parameter = logh_dr(1)(pos_eq) /
                                         logh_dr(1)(pos_pole);
...
#define FORMAT                                                        \
    std::setw(25) << std::right << std::setprecision(5) << std::fixed \
        ...
        std::cout << FORMAT << "Areal R = " << AR << " [" << AR * M2km
                  << "km]\n"
