...
int main(int argc, char** argv) {
  ...
  for (int i : {0, 1}) {
    ...
    auto A =
        space.get_domain(dom + 2)->integ(syst.give_val_def("intA")()(dom + 2),
                                         INNER_BC);
    areal_rs[i] = sqrt(A / 4. / acos(-1.));

    auto mirrsq =
        space.get_domain(dom + 2)->integ(syst.give_val_def("intMsq")()(dom + 2),
                                         INNER_BC);
    mirrs[i] = sqrt(mirrsq);
    spins[i] =
        space.get_domain(dom + 2)->integ(syst.give_val_def("intS")()(dom + 2),
                                         INNER_BC);
  ...
  double dtgamma_inf = space.get_domain(ndom - 1)->integ(
      syst.give_val_def("dtgamma")()(ndom - 1), OUTER_BC);
  double adm_inf =
  ...
  // Linear Momenta
  double Px =
      space.get_domain(ndom - 1)->integ(syst.give_val_def("intPx")()(ndom - 1),
                                        OUTER_BC);
  double Py =
      space.get_domain(ndom - 1)->integ(syst.give_val_def("intPy")()(ndom - 1),
                                        OUTER_BC);
  double Pz =
      space.get_domain(ndom - 1)->integ(syst.give_val_def("intPz")()(ndom - 1),
                                        OUTER_BC);
...
#define FORMAT                                                           \
  ...
  for (int i = 0; i <= 1; ++i) {
    ...
    std::cout << FORMAT1 << "Center_COM = " << "(" << xcom[i] << ", 0, 0)\n"
              ...
              << '\n'
              << FORMAT1 << "Coord R = "
              << bco_utils::get_radius(space.get_domain(nuc_doms[i] + 1), EQUI)
              << '\n';
