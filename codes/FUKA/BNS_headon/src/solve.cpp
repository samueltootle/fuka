...
int main(int argc, char** argv) {
  ...
  if (InitSolver::example_setup) {
    if (rank == 0) {
      ...
      bconfig.set_stage_map(MBINHEADONSTAGE);
      stages.fill(false);

      bconfig.set(BIN_PARAMS::DIST) = 50;
      bconfig.set_stage(STAGES::HEADON) = true;
    ...
    if (!(seq.is_set() || seq_bin.is_set()) &&
      ...
      if (seq.is_set())
        err =
            bns_xcts_sequence(bconfig, seq, resolution, InitSolver::outputdir);
      else
        err = bns_xcts_sequence(bconfig, seq_bin, resolution,
