...
struct reader_output {
    ...
    void operator()(config_t bconfig) {
        ...
        syst.add_def(space.ADAPTEDNS + 1,
                     "intS1 = A_ij * m1^i * s1^j / 8. / PI");
        syst.add_def(space.ADAPTEDBH + 1,
                     "intS2 = A_ij * m2^i * s2^j / 8. / PI");
        ...
        syst.add_def(ndom - 1,
                     "intJ = multr(A_ij * Morb^j * esurf^i) / 8 / PI");
        ...
        double ql_madm1 =
            std::accumulate(adm_mass1.begin(), adm_mass1.end(), 0.);
        ...
        double rout_ns =
            bco_utils::get_radius(space.get_domain(space.ADAPTEDNS + 1 +
                                                   bconfig(NSHELLS, BCO1)),
                                  EQUI);

        auto dHdx = syst.give_val_def("dH")();
        double dHdx1 = bco_utils::get_boundary_val(space.NS, dHdx, INNER_BC);
        double euler1 =
            bco_utils::get_boundary_val(space.NS,
                                        syst.give_val_def("firstint")(),
                                        INNER_BC);
        ...
        double r_bh =
            bco_utils::get_radius(space.get_domain(space.BH + 1), EQUI);
        double rout_bh =
    ...
    std::setw(25) << std::right << std::setprecision(5) << std::scientific \
        ...
        auto print_shells = [&](int dom_min, int dom_max) {
            ...
            for (int i = dom_min; i < dom_max; ++i) {
                ...
                Index pos(space.get_domain(i + 1)
                              ->get_radius()
                              .get_conf()
                              .get_dimensions());
                std::cout << FORMAT1 << shell
                          << space.get_domain(i + 1)->get_radius()(pos)
                          << std::endl;
                cnt++;
            }
        };
        ...
        std::cout << FORMAT1 << "Center_COM = " << "(" << NS_x_com
                  << ", 0, 0)\n"
...
int main(int argc, char** argv) {
    ...
    EOS_Function_Dispatcher::dispatch<reader_output>(bconfig,
                                                     eos_type,
                                                     bconfig);
