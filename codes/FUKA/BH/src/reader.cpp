...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::cerr << "Usage: ./reeader /<path>/<ID base name>.info "
                  << std::endl;
    ...
    double mirrsq =
        space.get_domain(2)->integ(syst.give_val_def("intMsq")()(2), INNER_BC);
    ...
    double A =
        space.get_domain(2)->integ(syst.give_val_def("intA")()(2), INNER_BC);
    ...
    auto [lapsemin, lapsemax] =
        bco_utils::get_field_min_max(lapse, 2, INNER_BC);
...
#define FORMAT1                                                       \
    ...
    std::cout << FORMAT1 << "RES = " << "[" << res_r << "," << res_t << ","
              << res_p << "]\n";
    std::cout << FORMAT1 << "Coord R_IN = "
              << bco_utils::get_radius(space.get_domain(0), OUTER_BC)
              << std::endl
              << FORMAT1 << "Coord R = "
              << bco_utils::get_radius(space.get_domain(1), OUTER_BC)
              << std::endl;
...
#define FORMAT                                                             \
    ...
    std::cout << FORMAT1 << "Mirr = " << mirr << "[" << bconfig(MIRR) << "]\n"
              ...
              << FORMAT << "Px = "
              << space.get_domain(ndom - 1)->integ(syst.give_val_def("intPx")()(
                                                       ndom - 1),
                                                   OUTER_BC)
              << std::endl
              << FORMAT << "Py = "
              << space.get_domain(ndom - 1)->integ(syst.give_val_def("intPy")()(
                                                       ndom - 1),
                                                   OUTER_BC)
              << std::endl
              << FORMAT << "Pz = "
              << space.get_domain(ndom - 1)->integ(syst.give_val_def("intPz")()(
                                                       ndom - 1),
                                                   OUTER_BC)
              << std::endl;
