...
std::array<std::vector<double>, NUM_OUT> KadathExportBHNS(
    int const npoints,
    double const* xx,
    double const* yy,
    double const* zz,
   ...
   * with only the extension being different */
    std::string kadath_filename = filename.substr(0, filename.size() - 5) +
                                  ".dat";
    ...
    Index I2(space.get_domain(space.BH + 2)
                 ->get_radius()
                 .get_conf()
                 .get_dimensions());
    ...
    for (int i = 0; i < npoints; ++i) {
        ...
        auto interp_f = [&](auto& ah_r, auto& extrap_r, auto& bh_ori) {
            ...
            double Phi =
                std::atan2(y_shifted,
                           (x_shifted - bh_ori));  // atan2 is needed here
