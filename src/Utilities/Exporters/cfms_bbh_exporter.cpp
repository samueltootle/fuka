...
CFMS_BBH_Exporter::CFMS_BBH_Exporter(CFMS_BBH_Exporter const& r) {
    std::string error_msg = export_utils::throw_no_multithreaded_support_error(
        "CFMS_BBH_Exporter - Copy Constructor");
    throw std::runtime_error(error_msg);
}

CFMS_BBH_Exporter& CFMS_BBH_Exporter::operator=(const CFMS_BBH_Exporter& b) {
    std::string error_msg = export_utils::throw_no_multithreaded_support_error(
        "CFMS_BBH_Exporter - Assignment operator");
    throw std::runtime_error(error_msg);
}
#endif
...
CFMS_BBH_Exporter::interp_ary_t CFMS_BBH_Exporter::interpolate_pointwise(
    double const& x,
    double const& y,
    double const& z,
    ...
    double x_shifted =
        x -
        (*bconfig)(
            Kadath::FUKA_Config::BIN_PARAMS::COM);  // shifting carpet [0,0] to
    // the initial center of mass
    double y_shifted =
        y -
        (*bconfig)(
            Kadath::FUKA_Config::BIN_PARAMS::COMY);  // shifting carpet [0,0] to
    // the initial center of mass
    ...
    auto interp_f = [&](auto& ah_r,
                        auto& extrap_r,
                        auto bh_ori,
                        auto BH_INNER_ADAPTED_IDX) {
        // Avoid division by "0"
        if (extrap_r == 0.)
            extrap_r = 1e-14;
        ...
        export_utils::spherical_turduck(quants,
                                        quant_vals,
                                        interp_order,
                                        delta_r_rel,
...
CFMS_BBH_Exporter::output_ary_t CFMS_BBH_Exporter::export_pointwise(
    double const& x,
    double const& y,
    double const& z,
    double const interpolation_offset,
