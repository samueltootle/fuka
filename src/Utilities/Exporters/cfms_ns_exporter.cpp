...
CFMS_NS_Exporter::CFMS_NS_Exporter(CFMS_NS_Exporter const& r) {
  std::string error_msg = export_utils::throw_no_multithreaded_support_error(
      "CFMS_NS_Exporter - Copy Constructor");
  throw std::runtime_error(error_msg);
}

CFMS_NS_Exporter& CFMS_NS_Exporter::operator=(const CFMS_NS_Exporter& b) {
  std::string error_msg = export_utils::throw_no_multithreaded_support_error(
      "CFMS_NS_Exporter - Assignment operator");
  throw std::runtime_error(error_msg);
}
#endif
...
CFMS_NS_Exporter::interp_ary_t CFMS_NS_Exporter::interpolate_pointwise(
    double const& x,
    double const& y,
    double const& z) {
...
CFMS_NS_Exporter::interp_ary_t CFMS_NS_Exporter::interpolate_pointwise_subset(
    double const& x,
    double const& y,
    double const& z,
...
CFMS_NS_Exporter::output_ary_t CFMS_NS_Exporter::export_pointwise(
    double const& x,
    double const& y,
    double const& z) {
  using namespace Kadath::FUKA_EOS;
  return EOS_Function_Dispatcher::dispatch<
      CFMS_NS_Exporter::export_pointwise_imp>(*bconfig, eos_type, *this, x, y,
                                              z);
}

CFMS_NS_Exporter::output_ary_t CFMS_NS_Exporter::export_pointwise_fluid_vars(
    double const& x,
    double const& y,
    double const& z) {
  using namespace Kadath::FUKA_EOS;
  return EOS_Function_Dispatcher::dispatch<
      CFMS_NS_Exporter::export_pointwise_fluid_vars_imp>(*bconfig, eos_type,
                                                         *this, x, y, z);
}

CFMS_NS_Exporter::output_ary_t
CFMS_NS_Exporter::export_pointwise_spacetime_vars(double const& x,
                                                  double const& y,
                                                  double const& z) {
  // Reset to NAN
