...
CFMS_NS_ISO_Exporter::output_ary_t CFMS_NS_ISO_Exporter::export_pointwise(
  ...
  return EOS_Function_Dispatcher::dispatch<
      CFMS_NS_ISO_Exporter::export_pointwise_imp>(*bconfig, eos_type, *this, x,
                                                  y, z);
}

// CFMS_NS_ISO_Exporter::output_ary_t
// CFMS_NS_ISO_Exporter::export_pointwise_fluid_vars(double const & x, double
// const & y, double const & z) {
...
// CFMS_NS_ISO_Exporter::output_ary_t
// CFMS_NS_ISO_Exporter::export_pointwise_spacetime_vars(double const & x,
...
//     */
//     {
//       const double tmp0 = sqrt(((xCart[0]) * (xCart[0])) + ((xCart[1]) *
//       (xCart[1])) + ((xCart[2]) * (xCart[2]))); xx0 = tmp0; if(std::fabs(xx0)
//       < 1e-12) xx0 = 1e-12; const REAL X = (std::fabs(xCart[0]) < 1e-12) ?
//       1e-12 : xCart[0]; xx1 = acos(xCart[2] / xx0); //theta (angle from Z to
//       xy plane) xx2 = atan2(xCart[1], X); // phi (angle from x to y axis)
//     }
...
//     const double betaSphericalU2 = -quant_vals[ISO_VARS::ISO_METRIC_OMEGA];
//     // phi
...
//     const double gammaSphericalDD22 = B * B * xx0 * xx0 * sin(xx1) *
//     sin(xx1);
...
//     const double tmp20 = ((tmp1) * (tmp1) * (tmp1) * (tmp1)) * ((xx0) * (xx0)
//     * (xx0) * (xx0)); const double tmp25 = ((tmp1) * (tmp1)); const double
//     tmp17 = tmp1 * tmp12 * tmp15; const double tmp21 = gammaSphericalDD00 *
//     tmp20; const double tmp23 = tmp13 * tmp4 * ((xx0) * (xx0) * (xx0)); const
//     double tmp26 = tmp12 * tmp25; const double tmp29 = -tmp15 * tmp9 - tmp25
//     * tmp9; const double tmp34 = tmp4 * tmp7; const double tmp41 = tmp15 *
//     tmp3 + tmp25 * tmp3; const double tmp52 = tmp1 * tmp12 * tmp4; const
//     double tmp68 = KSphericalDD00 * tmp20; const double tmp18 = (1.0 /
//     ((tmp10 * tmp12 * tmp13 + tmp10 * tmp17 + tmp11 * tmp12 * tmp13 + tmp11 *
//     tmp17) *
//                               (tmp10 * tmp12 * tmp13 + tmp10 * tmp17 + tmp11
//                               * tmp12 * tmp13 + tmp11 * tmp17)));
...
//     out_pw[OUTPUT_VARS::BETA1] = betaSphericalU0 * tmp0 * tmp1 +
//     betaSphericalU1 * tmp3 * tmp4 - betaSphericalU2 * tmp6 * tmp7;
//     out_pw[OUTPUT_VARS::BETA2] = betaSphericalU0 * tmp1 * tmp6 +
//     betaSphericalU1 * tmp4 * tmp9 + betaSphericalU2 * tmp0 * tmp7;
//     out_pw[OUTPUT_VARS::BETA3] = betaSphericalU0 * tmp4 - betaSphericalU1 *
//     tmp7; out_pw[OUTPUT_VARS::G11] = tmp19 * tmp21 + tmp19 * tmp24 + tmp19 *
//     tmp28 + ((tmp29) * (tmp29)) * tmp31 + 2 * tmp33 * tmp35 + 2 * tmp33 *
//     tmp37; out_pw[OUTPUT_VARS::G12] =
//         tmp21 * tmp40 + tmp24 * tmp40 + tmp28 * tmp40 + tmp29 * tmp31 * tmp41
//         + tmp35 * tmp43 + tmp35 * tmp44 + tmp37 * tmp43 + tmp37 * tmp44;
//     out_pw[OUTPUT_VARS::G13] =
//         gammaSphericalDD02 * tmp32 * tmp53 + gammaSphericalDD12 * tmp32 *
//         tmp47 + tmp0 * tmp56 + tmp0 * tmp58 + tmp39 * tmp49 + tmp39 * tmp51;
//     out_pw[OUTPUT_VARS::G22] = tmp21 * tmp61 + tmp24 * tmp61 + tmp28 * tmp61
//     + tmp31 * ((tmp41) * (tmp41)) + 2 * tmp35 * tmp63 + 2 * tmp37 * tmp63;
//     out_pw[OUTPUT_VARS::G23] =
//         gammaSphericalDD02 * tmp42 * tmp53 + gammaSphericalDD12 * tmp42 *
//         tmp47 + tmp49 * tmp64 + tmp51 * tmp64 + tmp56 * tmp6 + tmp58 * tmp6;
//     out_pw[OUTPUT_VARS::G33] = gammaSphericalDD00 * tmp66 + 2 *
//     gammaSphericalDD01 * tmp47 * tmp54 + gammaSphericalDD11 * tmp65;
//     out_pw[OUTPUT_VARS::K11] = tmp19 * tmp68 + tmp19 * tmp70 + tmp19 * tmp71
//     + ((tmp29) * (tmp29)) * tmp72 + 2 * tmp33 * tmp73 + 2 * tmp33 * tmp75;
//     out_pw[OUTPUT_VARS::K12] =
//         tmp29 * tmp41 * tmp72 + tmp40 * tmp68 + tmp40 * tmp70 + tmp40 * tmp71
//         + tmp43 * tmp73 + tmp43 * tmp75 + tmp44 * tmp73 + tmp44 * tmp75;
//     out_pw[OUTPUT_VARS::K13] =
//         KSphericalDD02 * tmp32 * tmp53 + KSphericalDD12 * tmp32 * tmp47 +
//         tmp0 * tmp79 + tmp0 * tmp80 + tmp39 * tmp77 + tmp39 * tmp78;
//     out_pw[OUTPUT_VARS::K22] = ((tmp41) * (tmp41)) * tmp72 + tmp61 * tmp68 +
//     tmp61 * tmp70 + tmp61 * tmp71 + 2 * tmp63 * tmp73 + 2 * tmp63 * tmp75;
//     out_pw[OUTPUT_VARS::K23] =
//         KSphericalDD02 * tmp42 * tmp53 + KSphericalDD12 * tmp42 * tmp47 +
//         tmp6 * tmp79 + tmp6 * tmp80 + tmp64 * tmp77 + tmp64 * tmp78;
//     out_pw[OUTPUT_VARS::K33] = KSphericalDD00 * tmp66 + 2 * KSphericalDD01 *
//     tmp47 * tmp54 + KSphericalDD11 * tmp65;
//   };
...
// CFMS_NS_ISO_Exporter::grid_ary_t
// CFMS_NS_ISO_Exporter::export_coordinate_array(
...
CFMS_NS_ISO_Exporter::export_pointwise__spherical(double const& x,
  ...
  using namespace Kadath::FUKA_EOS;
  return EOS_Function_Dispatcher::dispatch<
      CFMS_NS_ISO_Exporter::export_pointwise__spherical_imp>(*bconfig, eos_type,
                                                             *this, x, y, z);
}
}  // namespace Kadath::FUKA_Solvers