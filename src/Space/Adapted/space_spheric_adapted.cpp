...
Space_spheric_adapted::Space_spheric_adapted (Space_spheric_adapted const & sp) {
	// Copy Space values
  ...
  const Domain_shell_outer_adapted* pouter = dynamic_cast<const Domain_shell_outer_adapted*> (sp.get_domain(1)) ;
  domains[1] = new Domain_shell_outer_adapted(*this, *pouter) ;
  const Domain_shell_inner_adapted* pinner = dynamic_cast<const Domain_shell_inner_adapted*> (sp.get_domain(2)) ;
  domains[2] = new Domain_shell_inner_adapted(*this, *pinner) ;
