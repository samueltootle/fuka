...
Space_bin_ns::Space_bin_ns (Space_bin_ns const & sp) {
  ...
  this->NS2 = 3 + n_shells1;
  assert(NS2 == sp.NS2);

  this->ADAPTED1 = NS1 + n_shells1 + 1;
  assert(ADAPTED1 == sp.ADAPTED1);

  this->ADAPTED2 = NS2 + n_shells2 + 1;
  ...
  add_spherical_shells(NS1+1, n_shells1);
  ...
  add_spherical_shells(NS2+1, n_shells2);
